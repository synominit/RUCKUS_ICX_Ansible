...
def build_command(
    ...
    if enable is not None:
        if enable["primary_method"] is not None:
            if enable["state"] == "absent":
                cmd = "no aaa authentication enable default {0}".format(
                    enable["primary_method"]
                )
            else:
                cmd = "aaa authentication enable default {0}".format(
                    enable["primary_method"]
                )
    ...
    if login is not None:
        if login["primary_method"] is not None:
            if login["state"] == "absent":
                cmd = "no aaa authentication login default {0}".format(
                    login["primary_method"]
                )
            else:
                cmd = "aaa authentication login default {0}".format(
                    login["primary_method"]
                )
    ...
    if snmp_server is not None:
        if snmp_server["state"] == "absent":
            cmd = "no aaa authentication snmp-server default {0}".format(
                snmp_server["primary_method"]
            )
        else:
            cmd = "aaa authentication snmp-server default {0}".format(
                snmp_server["primary_method"]
            )
    ...
    if web_server is not None:
        if web_server["state"] == "absent":
            cmd = "no aaa authentication web-server default {0}".format(
                web_server["primary_method"]
            )
        else:
            cmd = "aaa authentication web-server default {0}".format(
                web_server["primary_method"]
            )
...
def main():
    ...
    dot1x_spec = dict(
        primary_method=dict(
            type="str", required=True, choices=["radius", "none"]
        ),
    ...
    required_one_of = [
        ["dot1x", "enable", "login", "snmp_server", "web_server"]
    ]
    ...
    commands = build_command(
        module, dot1x, enable, login, snmp_server, web_server
    )
