    ...
    for w in want:
        ...
        if egress_buffer_profile_7150:
            if egress_buffer_profile_7150["state"] == "present":
                commands.append(
                    "qos egress-buffer-profile %s port-share-level %s"
                    % (
                ...
                commands.append(
                    "no qos egress-buffer-profile %s port-share-level %s"
                    % (
                        egress_buffer_profile_7150["user_profile_name"],
                        egress_buffer_profile_7150["port_share_level"],
                    )
        ...
        if egress_buffer_profile_7X50:
            if egress_buffer_profile_7X50["state"] == "present":
                commands.append(
                    "qos egress-buffer-profile %s queue-share-level %s %s"
                    % (
                ...
                commands.append(
                    "no qos egress-buffer-profile %s queue-share-level %s %s"
                    % (
                        egress_buffer_profile_7X50["user_profile_name"],
                        egress_buffer_profile_7X50["queue_share_level"],
                        egress_buffer_profile_7X50["queue_number"],
        ...
        if egress_shape_ifg_bytes:
            if egress_shape_ifg_bytes["state"] == "present":
                commands.append(
                    "qos egress-shape-ifg-bytes %s"
                    % (egress_shape_ifg_bytes["value_in_bytes"])
                )
            elif egress_shape_ifg_bytes["state"] == "absent":
                commands.append(
                    "no qos egress-shape-ifg-bytes %s"
                    % (egress_shape_ifg_bytes["value_in_bytes"])
                )
        ...
        if ingress_buffer_profile:
            if ingress_buffer_profile["state"] == "present":
                commands.append(
                    "qos ingress-buffer-profile %s priority-group %s xoff %s"
                    % (
                ...
                commands.append(
                    "no qos ingress-buffer-profile %s priority-group %s xoff %s"
                    % (
                        ingress_buffer_profile["user_profile_name"],
                        ingress_buffer_profile["priority_group_number"],
                        ingress_buffer_profile["shared_level"],
        ...
        if mechanism:
            if mechanism["state"] == "present":
                commands.append(
                    "qos mechanism %s" % (mechanism["queueing_method"])
                )
            else:
                commands.append(
                    "no qos mechanism %s" % (mechanism["queueing_method"])
                )

        if monitor_queue_drop_counters:
            if monitor_queue_drop_counters["state"] == "present":
                commands.append(
                    "qos monitor-queue-drop-counters %s"
                    % (monitor_queue_drop_counters["port_id"])
                )
            else:
                commands.append(
                    "no qos monitor-queue-drop-counters %s"
                    % (monitor_queue_drop_counters["port_id"])
                )
        ...
        if queue_name:
            commands.append(
                "qos name %s %s"
                % (queue_name["old_name"], queue_name["new_name"])
            )

        if priority_to_pg:
            if priority_to_pg["state"] == "present":
                commands.append(
                    "qos priority-to-pg  qosp0 %s qosp1 %s qosp2 %s qosp3 %s qosp4 %s qosp5 %s qosp6 %s qosp7 %s"
                    % (
                        priority_to_pg["priority0"],
                        priority_to_pg["priority1"],
                        priority_to_pg["priority2"],
                        priority_to_pg["priority3"],
                        priority_to_pg["priority4"],
                        priority_to_pg["priority5"],
                        priority_to_pg["priority6"],
                        priority_to_pg["priority7"],
                    )
                )
            else:
                commands.append(
                    "no qos priority-to-pg  qosp0 %s qosp1 %s qosp2 %s qosp3 %s qosp4 %s qosp5 %s qosp6 %s qosp7 %s"
                    % (
                        priority_to_pg["priority0"],
                        priority_to_pg["priority1"],
                        priority_to_pg["priority2"],
        ...
        if profile:
            if profile["state"] == "present":
                commands.append(
                    "qos profile %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s"
                    % (
                        qos_profile[0],
                        profile["percentage7"],
                        qos_profile[1],
                ...
                commands.append(
                    "no qos profile %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s"
                    % (
                        qos_profile[0],
                        profile["percentage7"],
                        qos_profile[1],
        ...
        if scheduler_profile:
            if scheduler_profile["state"] == "present":
                commands.append(
                    "qos scheduler-profile %s mechanism %s"
                    % (
                        scheduler_profile["user_profile_name"],
                        scheduler_profile["scheduling_mechanism"],
                    )
                )

                commands.append(
                    "qos scheduler-profile %s profile qosp0 %s qosp1 %s qosp2 %s qosp3 %s qosp4 %s qosp5"
                    " %s qosp6 %s qosp7 %s"
                    % (
                        scheduler_profile["user_profile_name"],
                        scheduler_profile["wt0"],
                        scheduler_profile["wt1"],
                        scheduler_profile["wt2"],
                        scheduler_profile["wt3"],
                        scheduler_profile["wt4"],
                        scheduler_profile["wt5"],
                        scheduler_profile["wt6"],
                        scheduler_profile["wt7"],
                    )
                )
            else:
                commands.append(
                    "no qos scheduler-profile %s"
                    % scheduler_profile["user_profile_name"]
                )
        ...
        if sflow_set_cpu_rate_limit:
            if sflow_set_cpu_rate_limit["state"] == "present":
                commands.append(
                    "qos sflow-set-cpu-rate-limit %s %s"
                    % (
                        sflow_set_cpu_rate_limit["packet_rate"],
                        sflow_set_cpu_rate_limit["burst_size"],
                    )
                )
            else:
                command = "no qos sflow-set-cpu-rate-limit %s %s" % (
                    sflow_set_cpu_rate_limit["packet_rate"],
                    sflow_set_cpu_rate_limit["burst_size"],
                )
        ...
        if tagged_priority:
            if tagged_priority["state"] == "present":
                commands.append(
                    "qos tagged-priority %s %s"
                    % (tagged_priority["num"], tagged_priority["queue"])
                )
            else:
                commands.append(
                    "no qos tagged-priority %s %s"
                    % (tagged_priority["num"], tagged_priority["queue"])
                )
        ...
        if internal_trunk_queue:
            if internal_trunk_queue["state"] == "present":
                commands.append(
                    "qos-internal-trunk-queue %s %s"
                    % (
                        internal_trunk_queue["level"],
                        internal_trunk_queue["queue"],
                    )
                )
            else:
                commands.append(
                    "no qos-internal-trunk-queue %s %s"
                    % (
                        internal_trunk_queue["level"],
                        internal_trunk_queue["queue"],
                    )
        ...
        if attach_egress_buffer_profile:
            interface = "interface ethernet %s" % (
                attach_egress_buffer_profile["port"]
            )
            commands.append(interface)
            if attach_egress_buffer_profile["state"] == "present":
                cmd = "egress-buffer-profile %s" % (
                    attach_egress_buffer_profile["profile_name"]
                )
                commands.append(cmd)
            else:
                cmd = "no egress-buffer-profile %s" % (
                    attach_egress_buffer_profile["profile_name"]
                )
                commands.append(cmd)
        ...
        if dscp_priority:
            ...
            if dscp_priority["state"] == "present":
                commands.append(
                    "qos-tos map dscp-priority %s to %s"
                    % (dscp_value, dscp_priority["priority"])
                )
            else:
                commands.append(
                    "no qos-tos map dscp-priority %s to %s"
                    % (dscp_value, dscp_priority["priority"])
                )
    if have:
        if attach_egress_buffer_profile:
            ...
            if "no" in cmd:
                ...
                for k, v in prof.items():
                    if (
                        attach_egress_buffer_profile["port"] in k
                        and " ".join(strg) not in v
                    ):
...
def map_params_to_obj(module):
    obj = []
    params = {
        "egress_buffer_profile_7150": module.params[
            "egress_buffer_profile_7150"
        ],
        "egress_buffer_profile_7X50": module.params[
            "egress_buffer_profile_7X50"
        ],
        ...
        "monitor_queue_drop_counters": module.params[
            "monitor_queue_drop_counters"
        ],
        ...
        "attach_egress_buffer_profile": module.params[
            "attach_egress_buffer_profile"
        ],
...
def main():
    ...
    mechanism_spec = dict(
        queueing_method=dict(
            type="str", choices=["strict", "weighted", "mixed-sp-wrr"]
        ),
        state=dict(default="present", choices=["present", "absent"]),
    )
    ...
    queue_name_spec = dict(
        old_name=dict(type="str"), new_name=dict(type="str")
    )
    ...
    scheduler_profile_spec = dict(
        user_profile_name=dict(type="str"),
        scheduling_mechanism=dict(
            type="str", choices=["mixed-sp-wrr", "strict", "weighted"]
        ),
        wt0=dict(type="int"),
        wt1=dict(type="int"),
        wt2=dict(type="int"),
    ...
    argument_spec = dict(
        egress_buffer_profile_7150=dict(
            type="dict", options=egress_buffer_profile_7150_spec
        ),
        egress_buffer_profile_7X50=dict(
            type="dict", options=egress_buffer_profile_7x50_spec
        ),
        egress_shape_ifg_bytes=dict(
            type="dict", options=egress_shape_ifg_bytes_spec
        ),
        ingress_buffer_profile=dict(
            type="dict", options=ingress_buffer_profile_spec
        ),
        mechanism=dict(type="dict", options=mechanism_spec),
        monitor_queue_drop_counters=dict(
            type="dict", options=monitor_queue_drop_counters_spec
        ),
        ...
        sflow_set_cpu_rate_limit=dict(
            type="dict", options=sflow_set_cpu_rate_limit_spec
        ),
        tagged_priority=dict(type="dict", options=tagged_priority_spec),
        internal_trunk_queue=dict(
            type="dict", options=internal_trunk_queue_spec
        ),
        attach_egress_buffer_profile=dict(
            type="dict", options=attach_egress_buffer_profile_spec
        ),
