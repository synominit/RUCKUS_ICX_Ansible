...
def validate_power(module, power):
    ...
    if count > 1:
        module.fail_json(
            msg="power parameters are mutually exclusive: class,limit,priority,enabled"
        )
...
def map_config_to_obj(module):
    ...
    for item in set(match):
        obj = {
            ...
            "speed-duplex": parse_config_argument(
                configobj, item, "speed-duplex"
            ),
                ...
                for item in args:
                    ...
                    if value:
                        if item == "power":
                            ...
                            cmd = (
                                "spanning-tree" if item else "no spanning-tree"
                            )
                        else:
            ...
            if have_neighbors_lldp:
                ...
                for line in lines:
                    ...
                    if field[0].strip() == w["name"].split(" ")[1]:
                        for item in field:
                            match = re.search(
                                r'\s*\+\s+System name\s+:\s+"(.*)"', item, re.M
                            )
                            if match:
                                have_host.append(match.group(1))

                            match = re.search(
                                r'\s*\+\s+Port description\s+:\s+"(.*)"',
                                item,
                                re.M,
                            )
                            if match:
