    ...
    def populate(self):
        ...
        if data:
            ...
            self.facts["info"] = (
                ...
                + det["model"]
                + ", "
                + str(self.parse_serialnum(data, det["Unit"]))
            )
...
class Hardware(FactsBase):
    ...
    COMMANDS = ["show memory", "show flash"]
    ...
    def populate(self):
        ...
        if data:
            ...
            self.facts["filesystems_info"] = self.parse_filesystems_info(
                self.responses[1]
            )
        ...
        if data:
            if "Invalid input detected" in data:
                ...
                match = re.findall(
                    r"Stack unit ([0-9]+):\nTotal DRAM: ([0-9]+) bytes\n  Dynamic memory: ([0-9]+) bytes total, ([0-9]+) bytes free, ([0-9]+%) used",
                    data,
                    re.DOTALL,
                )
                if match:
                    ...
                    for i in range(len(match)):
                        self.facts["memtotal_kb"][
                            "Stack Unit " + str(match[i][0])
                        ] = {
                            "Total Memory": str(int(match[i][1]) / 1024) + "kb"
                        }
                        self.facts["memfree_kb"][
                            "Stack Unit " + str(match[i][0])
                        ] = {
                            "Free Memory": str(int(match[i][3]) / 1024) + "kb"
                        }
...
class Interfaces(FactsBase):
    ...
    def populate_mediatype(self, data):
        ...
        for line in lines:
            ...
            if match:
                self.facts["interfaces"][match.group(1)]["mediatype"] = (
                    match.group(2)
                )
    ...
    def populate_ipv4_interfaces(self, data):
        for key, value in data.items():
            ...
            primary_address = re.findall(
                r"Internet address is (\S+/\S+), .*$", value, re.M
            )
    ...
    def populate_ipv6_interfaces(self, data):
        ...
        for line in parts:
            ...
            if match:
                ...
                interface_type = " ".join(
                    (interface_type[1], interface_type[2])
                )
                continue
            ...
            if match_ipv4:
                self.add_ip_address(
                    match_ipv4.group(1), "ipv4", str(interface_type)
                )
                self.facts["interfaces"][key]["ipv4"].append(
                    {
                        "address": match_ipv4.group(1),
                        "subnet": match_ipv4.group(2),
                    }
                )

            if match:
                self.add_ip_address(
                    match.group(1), "ipv6", str(interface_type)
                )
                self.facts["interfaces"][key]["ipv6"].append(
                    {"address": match.group(1), "subnet": match.group(2)}
                )
    ...
    def add_ip_address(self, address, family, interface_type):
        if family == "ipv4":
            if not self.facts["all_ipv4_addresses"]:
                ...
                self.facts["all_ipv4_addresses"][interface_type].append(
                    address
                )
            else:
                self.facts["all_ipv4_addresses"][interface_type] = [address]
            ...
            if not self.facts["all_ipv6_addresses"]:
                ...
                self.facts["all_ipv6_addresses"][interface_type].append(
                    address
                )
            else:
                self.facts["all_ipv6_addresses"][interface_type] = [address]
    ...
    def parse_interfaces(self, data):
        ...
        for line in data.split("\n"):
            match = re.match(
                r"\s*(?:\d+)?[A-Za-z]+Ethernet\s*(\d+(?:/\d+)+)", line
            )
            if len(line) == 0:
                continue
