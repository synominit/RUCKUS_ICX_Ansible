...
def build_command(
    ...
    if commands is not None:
        if commands["state"] == "absent":
            cmd = "no aaa accounting commands {0} default start-stop".format(
                commands["privilege_level"]
            )
        else:
            cmd = "aaa accounting commands {0} default start-stop".format(
                commands["privilege_level"]
            )
...
def main():
    ...
    required_one_of = [
        ["commands", "dot1x", "exec_", "mac_auth", "system", "enable_console"]
    ]
    ...
    commands = build_command(
        module, commands, dot1x, exec_, mac_auth, system, enable_console
    )
