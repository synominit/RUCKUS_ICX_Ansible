...
__metaclass__ = type
from ansible_collections.synominit.icx.plugins.modules import icx_vlan
...
class TestICXVlanModule(TestICXModule):
    ...
    def setUp(self):
        ...
        self.mock_exec_command = patch(
            "ansible_collections.synominit.icx.plugins.modules.icx_vlan.exec_command"
        )
        self.exec_command = self.mock_exec_command.start()

        self.mock_load_config = patch(
            "ansible_collections.synominit.icx.plugins.modules.icx_vlan.load_config"
        )
        self.load_config = self.mock_load_config.start()

        self.mock_get_config = patch(
            "ansible_collections.synominit.icx.plugins.modules.icx_vlan.get_config"
        )
    ...
    def load_fixtures(self, commands=None):
        ...
        self.exec_command.return_value = (
            0,
            load_fixture("icx_vlan_config").strip(),
            None,
        )
    ...
    def test_icx_vlan_set_tagged_port(self):
        set_module_args(
            dict(
                name="test_vlan",
                vlan_id=5,
                tagged=dict(name=["ethernet 1/1/40 to 1/1/43", "lag 44"]),
            )
        )
    ...
    def test_icx_vlan_add_untagged_port(self):
        set_module_args(
            dict(
                name="test_vlan",
                vlan_id=3,
                interfaces=dict(name=["ethernet 1/1/10", "lag 5"]),
            )
        )
    ...
    def test_icx_vlan_purge_tagged_port(self):
        set_module_args(
            dict(
                vlan_id=3,
                tagged=dict(
                    name=["ethernet 1/1/40 to 1/1/42", "lag 44", "lag 44"],
                    purge=True,
                ),
            )
        )
    ...
    def test_icx_vlan_enable_ip_arp_inspection(self):
        ...
        if not self.ENV_ICX_USE_DIFF:
            result = self.execute_module(changed=True)
            expected_commands = ["vlan 5", "ip arp inspection vlan 5"]
            self.assertEqual(result["commands"], expected_commands)
        else:
            result = self.execute_module(changed=True)
            expected_commands = ["vlan 5", "exit", "ip arp inspection vlan 5"]
            self.assertEqual(result["commands"], expected_commands)
    ...
    def test_icx_vlan_enable_ip_dhcp_snooping(self):
        ...
        if not self.ENV_ICX_USE_DIFF:
            result = self.execute_module(changed=True)
            expected_commands = ["vlan 5", "ip dhcp snooping vlan 5"]
            self.assertEqual(result["commands"], expected_commands)
        else:
            result = self.execute_module(changed=True)
            expected_commands = ["vlan 5", "exit", "ip dhcp snooping vlan 5"]
            self.assertEqual(result["commands"], expected_commands)
    ...
    def test_icx_vlan_aggregate(self):
        aggregate = [
            dict(
                ...
                name="vlan_9",
                interfaces=dict(
                    name=["ethernet 1/1/40 to 1/1/43", "ethernet 1/1/44"]
                ),
            ...
            dict(
                ...
                name="vlan_7",
                interfaces=dict(
                    name=["ethernet 1/1/20 to 1/1/23", "ethernet 1/1/24"]
                ),
    ...
    def test_icx_vlan_stp_802_1w(self):
        ...
        set_module_args(
            dict(
                vlan_id=3,
                interfaces=dict(name=["ethernet 1/1/40"]),
                stp=stp_spec,
            )
        )
        if not self.ENV_ICX_USE_DIFF:
