...
from ansible_collections.synominit.icx.plugins.modules import icx_user
...
class TestICXSCPModule(TestICXModule):
    ...
    def setUp(self):
        ...
        self.mock_get_config = patch(
            "ansible_collections.synominit.icx.plugins.modules.icx_user.get_config"
        )
        self.get_config = self.mock_get_config.start()
        self.mock_load_config = patch(
            "ansible_collections.synominit.icx.plugins.modules.icx_user.load_config"
        )
        self.load_config = self.mock_load_config.start()
        self.mock_exec_command = patch(
            "ansible_collections.synominit.icx.plugins.modules.icx_user.exec_command"
        )
        ...
        def load_file(*args, **kwargs):
            ...
            for arg in args:
                if arg.params["check_running_config"] is True:
                    return load_fixture(
                        "show_running-config_include_username.txt"
                    ).strip()
                else:
                    return ""
    ...
    def test_icx_user_create_new_with_password_and_privilege(self):
        set_module_args(
            dict(name="ale6", privilege="5", configured_password="alethea123")
        )
        if not self.ENV_ICX_USE_DIFF:
    ...
    def test_icx_user_update_privilege(self):
        set_module_args(
            dict(name="ale1", privilege="0", configured_password="alethea123")
        )
        if not self.ENV_ICX_USE_DIFF:
    ...
    def test_icx_user_update_password(self):
        ...
        if not self.ENV_ICX_USE_DIFF:
            commands = [
                "username ale1 privilege 5 password alethea123"
            ]  # previous privilage will be added
            self.execute_module(commands=commands, changed=True)
        else:
            commands = [
    ...
    def test_icx_user_agregate(self):
        set_module_args(
            dict(
                aggregate=[
                    {"name": "ale6", "configured_password": "alethea123"},
                    {"name": "ale7", "configured_password": "alethea123"},
                ]
            )
        )
        if not self.ENV_ICX_USE_DIFF:
    ...
    def test_icx_user_only_update_changed_settings(self):
        ...
        if not self.ENV_ICX_USE_DIFF:
            commands = ["username ale3 privilege 4 password ale123"]
            self.execute_module(commands=commands, changed=True)
        else:
            commands = [
    ...
    def test_icx_user_purge(self):
        set_module_args(dict(aggregate=[{"name": "ale1"}], purge=True))
        if not self.ENV_ICX_USE_DIFF:
            commands = [
                "no username ale2",
                "no username ale3",
