...
__metaclass__ = type
from ansible_collections.synominit.icx.plugins.modules import icx_static_route
...
class TestICXStaticRouteModule(TestICXModule):
    ...
    def setUp(self):
        ...
        self.mock_get_config = patch(
            "ansible_collections.synominit.icx.plugins.modules.icx_static_route.get_config"
        )
        self.get_config = self.mock_get_config.start()

        self.mock_load_config = patch(
            "ansible_collections.synominit.icx.plugins.modules.icx_static_route.load_config"
        )
    ...
    def test_icx_static_route_remove(self):
        set_module_args(
            dict(prefix="172.16.10.0/24", next_hop="10.0.0.8", state="absent")
        )
        if not self.ENV_ICX_USE_DIFF:
            result = self.execute_module(changed=True)
            expected_commands = [
