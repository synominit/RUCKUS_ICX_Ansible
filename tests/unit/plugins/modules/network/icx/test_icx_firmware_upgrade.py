...
class TestICXfirmwareUpgradeModule(TestICXModule):
    ...
    def test_icx_firmware_upgrade_tftp(self):
        ...
        commands = [
            "copy tftp flash 10.198.137.217 SPR08095_b412ufi.bin secondary",
    ...
    def test_icx_firmware_upgrade_scp(self):
        ...
        commands = [
            "copy scp flash 10.198.137.217 /tftpboot/SPR08095_b412ufi.bin secondary",
            "write memory",
            "boot system flash secondary yes",
        ]
        result = self.execute_module(changed=True)
    ...
    def test_icx_firmware_upgrade_scp_diffPartition(self):
        ...
        commands = [
            "copy scp flash 10.198.137.217 /tftpboot/SPR08095_b412ufi.bin fips-ufi-primary-sig",
    ...
    def test_icx_firmware_upgrade_https(self):
        ...
        commands = [
            "copy https flash 172.26.66.68 SPR08095_B10ufi.bin secondary port 443",
            "boot system flash secondary yes",
        ]
        result = self.execute_module(changed=True)
        self.assertEqual(result["commands"], commands)
    ...
    def test_icx_firmware_upgrade_boot_onlyTrue_save_running_configTrue(self):
        set_module_args(
            dict(
                boot_only=True, save_running_config=True, partition="secondary"
            )
        )
