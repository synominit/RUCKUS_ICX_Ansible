...
__metaclass__ = type
from ansible_collections.synominit.icx.plugins.modules import icx_rate_limit
...
class TestICXRateLimitModule(TestICXModule):
    ...
    def setUp(self):
        ...
        self.mock_exec_command = patch(
            "ansible_collections.synominit.icx.plugins.modules.icx_rate_limit.exec_command"
        )
        # self.mock_run_commands = patch('ansible_collections.synominit.icx.plugins.modules.icx_rate_limit.run_commands')
        self.mock_load_config = patch(
            "ansible_collections.synominit.icx.plugins.modules.icx_rate_limit.load_config"
        )
        self.mock_get_config = patch(
            "ansible_collections.synominit.icx.plugins.modules.icx_rate_limit.get_config"
        )
    ...
    def test_icx_rate_limit_bum_absent(self):
        set_module_args(dict(rate_limit_bum=dict(minutes=7, state="absent")))
        result = self.execute_module(changed=True)
        expected_commands = ["no rate-limit-log 7"]
        self.assertEqual(result["commands"], expected_commands)

    def test_icx_rate_limit_bum_present(self):
        set_module_args(dict(rate_limit_bum=dict(minutes=7, state="present")))
        commands = ["rate-limit-log 7"]
        result = self.execute_module(changed=True)
        self.assertEqual(result["commands"], commands)

    def test_icx_rate_limit_arp_absent(self):
        set_module_args(dict(rate_limit_arp=dict(number=77, state="absent")))
        commands = ["no rate-limit-arp 77"]
        result = self.execute_module(changed=True)
        self.assertEqual(result["commands"], commands)

    def test_icx_rate_limit_arp_present(self):
        set_module_args(dict(rate_limit_arp=dict(number=77, state="present")))
    ...
    def test_icx_rate_limit_input_lag_preset(self):
        set_module_args(
            dict(
                rate_limit_input=dict(
                    port="1/1/2", lag="LAG1", average_rate=800
                ),
            )
        )
    ...
    def test_icx_rate_limit_input_absent(self):
        set_module_args(
            dict(
                rate_limit_input=dict(
                    port="1/1/2", average_rate=500, state="absent"
                )
            )
        )
        commands = [
            "interface ethernet 1/1/2",
            "no rate-limit input fixed 500",
        ]
        result = self.execute_module(changed=True)
        self.assertEqual(result["commands"], commands)
    ...
    def test_icx_rate_limit_input_preset(self):
        set_module_args(
            dict(rate_limit_input=dict(port="1/1/2", average_rate=500))
        )
    ...
    def test_icx_rate_limit_input_aggregate(self):
        aggregate = [
            dict(rate_limit_output=dict(lag="LAG1", port="1/1/2", value=800)),
            dict(rate_limit_output=dict(port="1/1/3", value=800)),
        ]
        set_module_args(dict(aggregate=aggregate))
        expected_commands = [
            "lag LAG1",
            "rate-limit output shaping ethernet 1/1/2 800",
            "interface ethernet 1/1/3",
            "rate-limit output shaping 800",
        ]
        result = self.execute_module(changed=True)
        self.assertEqual(result["commands"], expected_commands)
    ...
    def test_icx_rate_limit_input_absent_burst_size(self):
        ...
        commands = [
            "interface ethernet 1/1/2",
            "no rate-limit input fixed 500 burst 500",
        ]
        result = self.execute_module(changed=True)
        self.assertEqual(result["commands"], commands)
    ...
    def test_icx_rate_limit_input_preset_burst_size(self):
        set_module_args(
            dict(
                rate_limit_input=dict(
                    port="1/1/2", average_rate=500, burst_size=500
                )
            )
        )
        commands = [
            "interface ethernet 1/1/2",
            "rate-limit input fixed 500 burst 500",
        ]
        result = self.execute_module(changed=True)
        self.assertEqual(result["commands"], commands)
        ...
        set_module_args(
            dict(
                rate_limit_output=dict(
                    port="1/1/2", lag="LAG1", value=800, state="present"
                ),
    ...
    def test_icx_rate_limit_output_absent(self):
        set_module_args(
            dict(
                rate_limit_output=dict(port="1/1/2", value=500, state="absent")
            )
        )
        commands = [
            "interface ethernet 1/1/2",
            "no rate-limit output shaping 500",
        ]
        result = self.execute_module(changed=True)
        self.assertEqual(result["commands"], commands)
    ...
    def test_icx_rate_limit_output_absent_priority_queue(self):
        set_module_args(
            dict(
                rate_limit_output=dict(
                    port="1/1/2", value=500, priority_queue=5, state="absent"
                )
            )
        )
        commands = [
            "interface ethernet 1/1/2",
            "no rate-limit output shaping 500 priority 5",
        ]
        result = self.execute_module(changed=True)
        self.assertEqual(result["commands"], commands)
    ...
    def test_icx_rate_limit_output_preset_priority_queue(self):
        set_module_args(
            dict(
                rate_limit_output=dict(
                    port="1/1/2", value=500, priority_queue=5
                )
            )
        )
        commands = [
            "interface ethernet 1/1/2",
            "rate-limit output shaping 500 priority 5",
        ]
        result = self.execute_module(changed=True)
        self.assertEqual(result["commands"], commands)
    ...
    def test_icx_rate_limit_output_preset_broadcast_limit(self):
        set_module_args(
            dict(broadcast_limit=dict(port="1/1/2", kbps=50, log=True))
        )
    ...
    def test_icx_rate_limit_output_absent_broadcast_limit(self):
        set_module_args(
            dict(broadcast_limit=dict(port="1/1/2", kbps=50, log=False))
        )
        commands = [
            "interface ethernet 1/1/2",
            "no broadcast limit 50 kbps log",
        ]
        result = self.execute_module(changed=True, failed=False)
        self.assertEqual(result["commands"], commands)
    ...
    def test_icx_rate_limit_output_preset_unknown_unicast_limit(self):
        set_module_args(
            dict(unknown_unicast_limit=dict(port="1/1/2", kbps=50, log=True))
        )
        commands = [
            "interface ethernet 1/1/2",
            "unknown-unicast limit 50 kbps log",
        ]
        result = self.execute_module(changed=True, failed=False)
        self.assertEqual(result["commands"], commands)
    ...
    def test_icx_rate_limit_output_absent_unknown_unicast_limit(self):
        set_module_args(
            dict(unknown_unicast_limit=dict(port="1/1/2", kbps=50, log=False))
        )
        commands = [
            "interface ethernet 1/1/2",
            "no unknown-unicast limit 50 kbps log",
        ]
        result = self.execute_module(changed=True, failed=False)
        self.assertEqual(result["commands"], commands)
    ...
    def test_icx_rate_limit_output_preset_multicast_limit(self):
        set_module_args(
            dict(multicast_limit=dict(port="1/1/2", kbps=50, log=True))
        )
    ...
    def test_icx_rate_limit_output_absent_multicast_limit(self):
        set_module_args(
            dict(multicast_limit=dict(port="1/1/2", kbps=50, log=False))
        )
        commands = [
            "interface ethernet 1/1/2",
            "no multicast limit 50 kbps log",
        ]
        result = self.execute_module(changed=True, failed=False)
        self.assertEqual(result["commands"], commands)
