...
from ansible_collections.synominit.icx.plugins.modules import icx_acl_mac
...
class TestICXAclMacModule(TestICXModule):
    ...
    def setUp(self):
        ...
        self.mock_load_config = patch(
            "ansible_collections.synominit.icx.plugins.modules.icx_acl_mac.load_config"
        )
        self.load_config = self.mock_load_config.start()
        self.mock_exec_command = patch(
            "ansible_collections.synominit.icx.plugins.modules.icx_acl_mac.exec_command"
        )
    ...
    def test_icx_mac_permit_acl(self):
        ...
        expected_commands = [
            "mac access-list mac123",
            "permit 1111.2222.3333 4444.5555.6666 ether-type 0800",
        ]
        result = self.execute_module(changed=True)
        self.assertEqual(result["commands"], expected_commands)
    ...
    def test_icx_mac_deny_acl(self):
        ...
        expected_commands = [
            "mac access-list mac123",
            "deny 1111.2222.3333 4444.5555.6666 ether-type 0800 mirror",
        ]
        result = self.execute_module(changed=True)
        self.assertEqual(result["commands"], expected_commands)
