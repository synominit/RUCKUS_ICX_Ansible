...
from ansible_collections.synominit.icx.plugins.modules import icx_l3_interface
...
class TestICXFactsModule(TestICXModule):
    ...
    def setUp(self):
        ...
        self.mock_exec_command = patch(
            "ansible_collections.synominit.icx.plugins.modules.icx_l3_interface.exec_command"
        )
        self.exec_command = self.mock_exec_command.start()
        self.mock_get_config = patch(
            "ansible_collections.synominit.icx.plugins.modules.icx_l3_interface.get_config"
        )
        self.get_config = self.mock_get_config.start()
        self.mock_load_config = patch(
            "ansible_collections.synominit.icx.plugins.modules.icx_l3_interface.load_config"
        )
        ...
        def load_from_file(*args, **kwargs):
            ...
            for arg in args:
                if arg.params["check_running_config"] is True:
                    return load_fixture(
                        "show_running-config_begin_interface"
                    ).strip()
                else:
                    return ""
    ...
    def test_icx_l3_interface_set_ipv6(self):
        set_module_args(
            dict(name="ethernet 1/1/1", ipv6="2001:db8:85a3:0:0:0:0:1/64")
        )
        if not self.ENV_ICX_USE_DIFF:
            commands = [
                "interface ethernet 1/1/1",
    ...
    def test_icx_l3_interface_remove_ipv6(self):
        set_module_args(
            dict(
                name="ethernet 1/1/1",
                ipv6="2001:db8:85a3:0:0:0:0:0/64",
                ipv4="192.168.1.1/24",
                state="absent",
            )
        )
