...
class TestICXAaaAuthenticationModule(TestICXModule):
    ...
    def setUp(self):
        ...
        self.mock_load_config = patch(
            "ansible_collections.commscope.icx.plugins.modules.icx_aaa_authentication.load_config"
        )
        self.load_config = self.mock_load_config.start()
        self.mock_exec_command = patch(
            "ansible_collections.commscope.icx.plugins.modules.icx_aaa_authentication.exec_command"
        )
    ...
    def test_icx_aaa_authentication_all_options_no_backup_remove(self):
        ...
        expected_commands = [
            "no aaa authentication dot1x default radius",
    ...
    def test_icx_aaa_authentication_dot1x_enable(self):
        ...
        set_module_args(
            dict(
                dot1x=dict(primary_method="radius", state="present"),
                enable=dict(
                    primary_method="enable",
                    backup_method_list=["line", "local"],
                    state="present",
                ),
            )
        )
        expected_commands = [
            "aaa authentication dot1x default radius",
            "aaa authentication enable default enable line local",
        ]
        result = self.execute_module(changed=True)
        self.assertEqual(result["commands"], expected_commands)
    ...
    def test_icx_aaa_authentication_login_snmp_server(self):
        ...
        set_module_args(
            dict(
                login=dict(privilege_mode=True, state="present"),
                snmp_server=dict(primary_method="enable", state="present"),
            )
        )
        expected_commands = [
    ...
    def test_icx_aaa_authentication_web_server(self):
        ...
        set_module_args(
            dict(web_server=dict(primary_method="tacacs", state="present"))
        )
    ...
    def test_icx_aaa_authentication_invalid_arg_dot1x(self):
        ...
        set_module_args(
            dict(dot1x=dict(primary_method="aaa", state="present"))
        )
