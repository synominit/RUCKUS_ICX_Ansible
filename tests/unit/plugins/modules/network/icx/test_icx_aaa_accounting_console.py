...
class TestICXAaaAccountingModule(TestICXModule):
    ...
    def setUp(self):
        ...
        self.mock_load_config = patch(
            "ansible_collections.synominit.icx.plugins.modules.icx_aaa_accounting_console.load_config"
        )
        self.load_config = self.mock_load_config.start()
        self.mock_exec_command = patch(
            "ansible_collections.synominit.icx.plugins.modules.icx_aaa_accounting_console.exec_command"
        )
    ...
    def test_icx_aaa_accounting_commands_dot1x(self):
        ...
        set_module_args(
            dict(
                commands=dict(
                    privilege_level=4, primary_method="radius", state="present"
                ),
                dot1x=dict(primary_method="none", state="present"),
            )
        )
        expected_commands = [
            "aaa accounting commands 4 default start-stop radius",
    ...
    def test_icx_aaa_accounting_exec_mac_auth_system(self):
        ...
        set_module_args(
            dict(
                exec_=dict(primary_method="tacacs+"),
                mac_auth=dict(primary_method="radius"),
                system=dict(primary_method="tacacs+"),
            )
        )
        expected_commands = [
    ...
    def test_icx_aaa_accounting_invalid_args_exec(self):
        ...
        set_module_args(
            dict(exec_=dict(primary_method="aaa", state="present"))
        )
        result = self.execute_module(failed=True)
