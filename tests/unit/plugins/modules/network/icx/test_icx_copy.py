...
class TestICXSCPModule(TestICXModule):
    ...
    def load_fixtures(self, commands=None):
        self.exec_commands.return_value = (
            0,
            load_fixture("icx_copy.txt").strip(),
            None,
        )
        ...
        if commands is not None:
            ...
            self.mock_exec_scp.return_value = load_fixture(
                "icx_copy.txt"
            ).strip()
            self.mock_run_commands.return_value = load_fixture(
                "icx_copy.txt"
            ).strip()
        else:
    ...
    def test_icx_https_upload_running(self):
        ...
        commands = [
            "copy running-config https fileserver.alethea.in filestorage/test/upload_running"
        ]
        self.execute_module(commands=commands)
    ...
    def test_icx_https_download_running(self):
        ...
        commands = [
            "copy https running-config fileserver.alethea.in filestorage/test/running.conf"
        ]
    ...
    def test_icx_https_upload_startup(self):
        ...
        commands = [
            "copy startup-config https fileserver.alethea.in filestorage/test/upload_startup"
        ]
        self.execute_module(commands=commands)
    ...
    def test_icx_https_download_startup(self):
        ...
        commands = [
            "copy https startup-config fileserver.alethea.in filestorage/test/startup.conf"
        ]
        self.execute_module(commands=commands, changed=True)
    ...
    def test_icx_https_upload_primary(self):
        ...
        commands = [
            "copy startup-config https fileserver.alethea.in filestorage/test/upload_primary"
        ]
        self.execute_module(failed=True)
    ...
    def test_icx_https_download_primary(self):
        set_module_args(
            dict(
                ...
                protocol="https",
                remote_server="fileserver.alethea.in",
                remote_filename="filestorage/test/primary.bin",
            )
        )
        commands = [
            "copy https flash fileserver.alethea.in filestorage/test/primary.bin primary"
        ]
        self.execute_module(commands=commands, changed=True)
    ...
    def test_icx_https_upload_secondary(self):
        ...
        commands = [
            "copy flash https fileserver.alethea.in filestorage/test/upload_secondary secondary"
        ]
        self.execute_module(failed=True)
    ...
    def test_icx_https_download_secondary(self):
        ...
        commands = [
            "copy https flash fileserver.alethea.in filestorage/test/secondary.bin secondary"
        ]
        self.execute_module(commands=commands, changed=True)
