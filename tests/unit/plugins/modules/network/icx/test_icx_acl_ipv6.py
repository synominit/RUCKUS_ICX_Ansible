...
class TestICXAclIpv6Module(TestICXModule):
            ...
            dict(
                ...
                rules=[
                    (
                        dict(
                            remark=dict(
                                comment_text="Permits ipv6 traffic from any source to any destination"
                            ),
                            seq_num="10",
                            rule_type="permit",
                    ...
                    (
                        dict(
                            ...
                            source=dict(
                                ipv6_prefix_prefix_length="2001:DB8::/64"
                            ),
                            destination=dict(any="yes"),
                            icmp_num="25",
                            dscp_matching="21",
                    ...
                    (
                        dict(
                            remark=dict(
                                comment_text="Denies tcp traffic from host 2001:DB8:e0ac::2 to 2001:DB8:e0aa:0::24"
                            ),
                            seq_num="30",
                            rule_type="deny",
    ...
    def test_icx_acl_ip_ipv6_enable_accounting(self):
        ...
        expected_commands = ["ipv6 access-list acl1", "enable accounting"]
        result = self.execute_module(changed=True)
        self.assertEqual(result["commands"], expected_commands)
    ...
    def test_icx_acl_ip_ipv6_disable_accounting(self):
        ...
        expected_commands = ["ipv6 access-list acl1", "no enable accounting"]
        result = self.execute_module(changed=True)
        self.assertEqual(result["commands"], expected_commands)
