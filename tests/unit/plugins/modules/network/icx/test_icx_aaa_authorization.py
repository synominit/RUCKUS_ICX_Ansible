...
class TestICXAaaAuthorizationModule(TestICXModule):
    ...
    def test_icx_aaa_authorization_all_options_no_backup_remove(self):
        ...
        set_module_args(
            dict(
                ...
                commands=dict(
                    privilege_level=0, primary_method="radius", state="absent"
                ),
    ...
    def test_icx_aaa_authorization_coa_enable_coa_ignore(self):
        ...
        set_module_args(
            dict(
                coa_enable="present",
                coa_ignore=dict(request="dm-request", state="present"),
            )
        )
        expected_commands = [
            "aaa authorization coa enable",
    ...
    def test_icx_aaa_authorization_commands_exec_(self):
        ...
        set_module_args(
            dict(
                commands=dict(
                    privilege_level=0, primary_method="radius", state="present"
                ),
                exec_=dict(primary_method="radius", state="present"),
            )
        )
        expected_commands = [
            "aaa authorization commands 0 default radius",
    ...
    def test_icx_aaa_authorization_invalid_args_commands(self):
        ...
        set_module_args(
            dict(
                commands=dict(
                    privilege_level=2, primary_method="radius", state="present"
                )
            )
        )
        result = self.execute_module(failed=True)
