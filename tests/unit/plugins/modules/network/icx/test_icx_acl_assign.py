...
from ansible_collections.synominit.icx.plugins.modules import icx_acl_assign
...
class TestICXAclAssignModule(TestICXModule):
    ...
    def setUp(self):
        ...
        self.mock_load_config = patch(
            "ansible_collections.synominit.icx.plugins.modules.icx_acl_assign.load_config"
        )
        self.load_config = self.mock_load_config.start()
        self.mock_exec_command = patch(
            "ansible_collections.synominit.icx.plugins.modules.icx_acl_assign.exec_command"
        )
    ...
    def test_icx_acl_assign_all_options(self):
        ...
        set_module_args(
            dict(
                ...
                ipv6_access_group=dict(
                    acl_name="scale12", in_out="in", lag="7"
                ),
                mac_access_group=dict(
                    mac_acl_name="mac_acl1",
                    vlan=dict(vlan_num="10", interfaces=["ethernet 1/1/3"]),
                    logging="enable",
                ),
                default_acl=dict(
                    ip_type="ipv4", acl_name="guest", in_out="in"
                ),
            )
        )
        expected_commands = [
    ...
    def test_icx_acl_assign_all_options_remove(self):
        ...
        set_module_args(
            dict(
                ...
                ipv6_access_group=dict(
                    acl_name="scale12", in_out="in", lag="7", state="absent"
                ),
                mac_access_group=dict(
                    mac_acl_name="mac_acl1",
                    vlan=dict(vlan_num="10", interfaces=["ethernet 1/1/3"]),
    ...
    def test_icx_acl_assign_ip_access_group(self):
        ...
        set_module_args(
            dict(
                ip_access_group=dict(
                    in_out="in", ethernet="1/1/2", frag_deny="yes"
                )
            )
        )
        expected_commands = [
    ...
    def test_icx_acl_assign_ipv6_access_group(self):
        ...
        set_module_args(
            dict(
                ipv6_access_group=dict(
                    acl_name="scale123",
                    in_out="in",
                    vlan=dict(vlan_num="555", interfaces=["ethernet 1/1/2"]),
                    logging="enable",
                )
            )
        )
        expected_commands = [
            "vlan 555",
            "ipv6 access-group scale123 in ethernet 1/1/2 logging enable",
            "exit",
        ]
        result = self.execute_module(changed=True)
        self.assertEqual(result["commands"], expected_commands)
    ...
    def test_icx_acl_assign_mac_access_group(self):
        ...
        set_module_args(
            dict(mac_access_group=dict(mac_acl_name="mac_acl1", lag="25"))
        )
        expected_commands = [
            "interface lag 25",
