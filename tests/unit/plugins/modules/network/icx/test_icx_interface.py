...
__metaclass__ = type
from ansible_collections.synominit.icx.plugins.modules import icx_interface
...
class TestICXInterfaceModule(TestICXModule):
    ...
    def setUp(self):
        ...
        self.mock_exec_command = patch(
            "ansible_collections.synominit.icx.plugins.modules.icx_interface.exec_command"
        )
        self.exec_command = self.mock_exec_command.start()

        self.mock_load_config = patch(
            "ansible_collections.synominit.icx.plugins.modules.icx_interface.load_config"
        )
        self.load_config = self.mock_load_config.start()

        self.mock_get_config = patch(
            "ansible_collections.synominit.icx.plugins.modules.icx_interface.get_config"
        )
            ...
            for arg in args:
                if arg.params["check_running_config"] is True:
                    self.exec_command.return_value = (
                        0,
                        load_fixture("icx_interface_config.cfg").strip(),
                        None,
                    )
    ...
    def test_icx_interface_remove(self):
        ...
        if not self.ENV_ICX_USE_DIFF:
            result = self.execute_module(changed=True)
            self.assertEqual(
                result["commands"], ["no interface ethernet 1/1/1"]
            )
        else:
            result = self.execute_module(changed=True)
            self.assertEqual(
    ...
    def test_icx_interface_disable(self):
        ...
        if not self.ENV_ICX_USE_DIFF:
            result = self.execute_module(changed=True)
            self.assertEqual(
                result["commands"], ["interface ethernet 1/1/1", "disable"]
            )
        else:
            result = self.execute_module(changed=True)
            self.assertEqual(
    ...
    def test_icx_interface_aggregate(self):
        ...
        aggregate = [
            dict(
                name="ethernet 1/1/9",
                description="welcome port9",
                speed="1000-full",
                power=power,
            ),
            dict(
                name="ethernet 1/1/10",
                description="welcome port10",
                speed="1000-full",
                power=power,
            ),
        ]
    ...
    def test_icx_interface_lag_config(self):
        set_module_args(
            dict(name="lag 11", description="lag ports of id 11", speed="auto")
        )
        if not self.ENV_ICX_USE_DIFF:
            result = self.execute_module(changed=True)
            expected_commands = [
    ...
    def test_icx_interface_loopback_config(self):
        set_module_args(
            dict(
                name="loopback 10", description="loopback ports", enabled=True
            )
        )
        if not self.ENV_ICX_USE_DIFF:
            result = self.execute_module(changed=True)
    ...
    def test_icx_interface_state_up_cndt(self):
        set_module_args(
            dict(name="ethernet 1/1/1", state="up", tx_rate="ge(0)")
        )
        if not self.ENV_ICX_USE_DIFF:
    ...
    def test_icx_interface_lldp_neighbors_cndt(self):
        set_module_args(
            dict(
                ...
                neighbors=[
                    dict(
                        port="GigabitEthernet1/1/48", host="ICX7150-48 Router"
                    )
                ],
            )
        )
